...
(()=>{'use strict';
const SCOPE='company',REPORT_KEY='performance-report-v2',COMPOSER_KEY='performance-report-composer-v1';
...
async function applyRemote(row,announce=false){
 ...
 try{
  if(row.data_key===REPORT_KEY){await window.PJT_APPLY_SHARED_REPORT?.(row.data)}
...
window.PJT_SHARED_REPORT_LOAD=async()=>{await ensureStarted();return cache.get(REPORT_KEY)?.data||null};
window.PJT_SHARED_REPORT_SAVE=snapshot=>scheduleSave(REPORT_KEY,snapshot,850);
